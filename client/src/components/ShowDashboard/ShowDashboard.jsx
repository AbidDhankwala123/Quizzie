          ...
          {dashboardQuizzes && dashboardQuizzes.length>0 && dashboardQuizzes.map((quiz, idx) => {
            ...
            return (
              <div className={styles.square_quiz} key={idx}>
                <div>
                  <span title={quiz.quizName} className={styles.quiz_name} >{quiz.quizName}</span>
                  <span className={styles.impressions} >{quiz.totalImpressions} <img style={{ position: "relative", top: "2px" }} src={eyeIcon} alt="eye_icon" /></span>
                </div>
                <p className={styles.created_date}>Created on : {formattedDate}</p>
              </div>
            )
